...
const excelPath = path.join(__dirname, '../Tabelas 2025/1. Demografia/Tab 1.7 Razão de Dependência - 2021 a 2025.xlsx');

// Lê o arquivo Excel
console.log('📖 Lendo arquivo Excel: Tab 1.7 Razão de Dependência...');
...
for (let i = 5; i < data.length; i++) {
  ...
  if (municipio && typeof municipio === 'string' && municipio.trim() && !isNaN(valor2024)) {
    ...
    razaoDependencia2024[municipio.trim()] = valor;
    console.log(`  ${municipio.trim()}: ${valor}`);
  }
}

console.log(`\n✓ Total de municípios processados: ${Object.keys(razaoDependencia2024).length}`);

// Salva em JSON
const outputPath = path.join(__dirname, '../data/razao_dependencia_2024.json');
fs.writeFileSync(outputPath, JSON.stringify(razaoDependencia2024, null, 2), 'utf-8');

console.log(`\n✅ Arquivo salvo em: ${outputPath}`);
console.log(`   Amostra de dados:`);
const amostra = Object.entries(razaoDependencia2024).slice(0, 3);
