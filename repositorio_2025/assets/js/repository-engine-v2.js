(function () {
  ...
  function matchFilters(row) {
    ...
    const haystack = normalizeText(
      [row.ano, row.eixo, row.tema, row.regiao, row.municipio, row.codigo, row.titulo, row.indicador].join(" ")
    );
  ...
  function renderResults() {
    const html = state.filtered
      .map((row) => {
        ...
        return `<tr><td>${escapeHtml(row.ano)}</td><td>${escapeHtml(row.eixo)}</td><td>${escapeHtml(row.tema)}</td><td>${escapeHtml(row.regiao)}</td><td>${escapeHtml(row.municipio)}</td><td>${escapeHtml(row.codigo)}</td><td>${escapeHtml(row.titulo)}</td><td><a href="${href}" target="_blank" rel="noopener noreferrer">${fileName}</a></td></tr>`;
      })
      .join("");
  ...
  function bindEvents() {
    ...
    els.btnLimpar.addEventListener("click", () => {
      state.filters = { ano: "", eixo: "", tema: "", regiao: "", municipio: "", busca: "" };
      els.ano.value = els.eixo.value = els.tema.value = els.regiao.value = els.municipio.value = "";
      els.busca.value = "";
      applyFilters();
    });
  }
  ...
  async function init() {
    try {
      await loadData();
      const urlRegiao = getQueryParam("regiao");
      if (urlRegiao) {
        state.filters.regiao = urlRegiao;
      }
      ...
      fillSelect(els.regiao, uniqueValues(state.raw, "regiao"), state.filters.regiao);
      fillSelect(els.municipio, uniqueValues(state.raw, "municipio"), "");

      if (state.filters.regiao) {
        els.regiao.value = state.filters.regiao;
      }
      ...
      els.empty.textContent = "Erro ao carregar dados.";
    }
  }
