...
def infer_region_municipality(filename, path_parts):
  # Se houver uma pasta "municipios" ou similar na estrutura, extrai municipality
  # Caso contrário, deixa vazio (a maioria são tabelas estaduais)
  region = ""
  municipality = ""
  
  # Procura por padrões de região/município nas pastas
  path_lower = [p.lower() for p in path_parts]
  
  # Se houver "municipio" ou "municipal" nas pastas, tenta extrair do nome
  if any("munic" in p for p in path_lower):
    base = Path(filename).stem
    # Extrai o último token que não é número ou palabra fraca
